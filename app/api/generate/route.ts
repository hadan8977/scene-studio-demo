...
export async function POST(request:Request){
 ...
 const encoder=new TextEncoder();const stream=new ReadableStream<Uint8Array>({
  async start(controller){const emit=(event:unknown)=>{if(!lifecycle.signal.aborted)controller.enqueue(encoder.encode(`data: ${JSON.stringify(event)}\n\n`));};
   try{if(isInjection(body.input))emit({type:'result',result:validateScene(emptyScene(),body.context,body.input),timing:{ttft:null,understanding:null,total:0,attempts:0},model:'本地验证器'});else await generate(body,key,emit,lifecycle.signal);}
   catch(error){try{controller.enqueue(encoder.encode(`data: ${JSON.stringify({type:'error',message:lifecycle.signal.aborted?'等待超过30秒，输入已保留，请重试':error instanceof Error?error.message:'生成失败，请重试'})}\n\n`));}catch{}}
   finally{clearTimeout(timer);try{controller.close();}catch{}}
  },cancel(){clearTimeout(timer);lifecycle.abort();}
 });
