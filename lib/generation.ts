...
export type Timing={ttft:number|null;understandingStart?:number|null;understanding:number|null;total:number;attempts:number};
...
export async function generate(input:GenerateInput,key:string,emit:(event:GenerationEvent)=>void,signal:AbortSignal,fetcher:typeof fetch=fetch) {
 ...
 let ttft:number|null=null,tUnd:number|null=null,tUndStart:number|null=null;
 ...
 for(let attempt=1;attempt<=2;attempt++){
  ...
  try{parsed=parseScene(JSON.parse(text.trim().replace(/^```(?:json)?\s*|\s*```$/g,'')));}catch{
   if(attempt===1){tUnd=null;tUndStart=null;emit({type:'retry',text:'格式需要整理，正在重试一次'});continue;}throw new Error('模型输出格式仍不完整，输入已保留，请重试');
  }
  ...
  emit({type:'result',result,timing:{ttft,understandingStart:tUndStart,understanding:tUnd,total:(performance.now()-started)/1000,attempts:attempt},model:model.id});return;
 }
}
