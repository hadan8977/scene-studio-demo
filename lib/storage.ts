...
export type SavedScene = {
  ...
  updatedAt: string;
  profileId?: ProfileId;
  origin?: import('./nonvoice.ts').SceneOrigin;
};
      ...
      .filter((x) => {
        try {
          return (
            ...
            ['live', 'example'].includes(x.source) &&
            typeof x.updatedAt === 'string' &&
            Array.isArray(x.result?.decisions) &&
            !!parseScene(x.result.scene)
          );
        } catch {
          return false;
        }
...
export function upsertSaved(
  ...
  return [item, ...items.filter((x) => x.id !== item.id)].slice(0, 60);
}
