...
test('stream handles split UTF-8 and SSE chunks, reports real timings and valid scene', async () => {
  const h = harness([
      modelStream(JSON.stringify(exampleScene('rain', context))),
    ]),
  ...
  assert.equal(result.result.savable, true);
  assert.ok(events.some((e) => e.type === 'understanding'));
  assert.ok(result.timing.understanding !== null);
  assert.ok(result.timing.understandingStart != null);
  assert.ok(result.timing.understandingStart! >= result.timing.ttft!);
  assert.ok(result.timing.understanding! >= result.timing.understandingStart!);
...
test('invalid JSON retries exactly once and surfaces retry event', async () => {
  const h = harness([
      modelStream('not JSON'),
      modelStream(JSON.stringify(exampleScene('wait', context))),
    ]),
    events: GenerationEvent[] = [];
  await generate(
    payload,
...
test('second malformed response ends with error, no false result', async () => {
  const h = harness([modelStream('wrong'), modelStream('{}')]),
    events: GenerationEvent[] = [];
...
test('provider disconnect is surfaced, no partial actions emitted', async () => {
  ...
  const r = new Response(
    new ReadableStream({
      start(c) {
        c.enqueue(encoder.encode('data: {"error":{"message":"failed"}}\n\n'));
        c.close();
      },
    }),
  );
...
test('abort signal is forwarded to provider and ends generation', async () => {
  ...
  a.abort();
  const fetcher = (async (_url: RequestInfo | URL, opts?: RequestInit) => {
    opts?.signal?.throwIfAborted();
    throw new Error('not aborted');
  }) as typeof fetch;
  await assert.rejects(
    generate(payload, 'secret', () => {}, a.signal, fetcher),
    /abort/i,
  );
});
test('input validation rejects invalid context and malformed draft', () => {
  assert.throws(() =>
    inputFrom({ ...payload, context: { driving: 'yes', profile: 'none' } }),
  );
  assert.throws(() => inputFrom({ ...payload, input: 'x'.repeat(1201) }));
  assert.throws(() => inputFrom({ ...payload, currentScene: {} }));
...
test('understanding parser supports escaped quotes without inventing completion', () => {
  assert.deepEqual(
    understandingFromPartial('{"understanding":"他说\\"安静\\"'),
    { text: '他说"安静"', complete: false },
  );
  assert.deepEqual(understandingFromPartial('{"understanding":"安静","name"'), {
    text: '安静',
    complete: true,
  });
});
