...
本版本直接以用户上传的 Figma Make「Card Stack」源代码为前端基础。保留原稿的 HmiFrame、PopupSurface、ManagerSurface、SceneCard、Composer 和主题样式，接入本项目的真实生成服务、96 条能力校验与持久化。上一版 LUMA 前端及其样式、图形已移除。
...
默认在原稿的导航背景与命令栏等待输入，示例结果均明确标注。示例模式仅支持预设案例及少量局部续改，不会将预设数据冒充为模型输出。
...
37项测试覆盖能力名、枚举、范围、步长、行驶限制、禁止值、无法表达的触发条件、负面偏好、局部修改、澄清、存储恢复、SSE分块、重试、错误与取消。

生成服务测试使用受控响应夹具，不代表真实模型的质量和时延。真实模型联调需自行配置密钥。10条 React DOM 交互旅程覆盖保存跳转、续改、追问、删除、行驶限制、档案持久化和连续提交竞争等路径；真实浏览器视觉验收和 WebMCP 环境验证尚未完成。
