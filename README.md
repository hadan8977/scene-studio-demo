...
LUMA 车机场景应用：1920×1080设备框、导航与音乐情境、单张悬浮生成卡，以及独立的场景管理/创建/编辑流程。
...
生成服务测试使用受控响应夹具，不代表真实模型的质量和时延。真实模型联调需自行配置密钥。React DOM交互测试覆盖保存跳转、续改、追问、删除、行驶限制等路径；真实浏览器视觉验收和 WebMCP 环境验证尚未完成。
