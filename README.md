...
本版本直接以用户上传的 Figma Make「Card Stack」源代码为前端基础。保留原稿的 HmiFrame、PopupSurface、ManagerSurface、SceneCard、Composer 和主题样式，接入本项目的真实生成服务、114 条能力校验与持久化。上一版 LUMA 前端及其样式、图形已移除。

1920×1080倾斜显示器、底部居中的520px提案和600px命令栏、288px应用侧栏、560px应用内编辑弹层，均沿用原稿。详见[界面来源与适配说明](docs/figma-make-foundation.md)。
...
本轮改动与边界见 [产品评审记录](docs/demo-review-v16.md)。地图数据与许可见 [地图说明](public/map/README.md)。
...
- 局部修改时保留其他动作，例如“灯再暗一点”；直接续聊、快捷修改和刚改反馈在同卡内完成。
- 上海徐汇滨江真实街区地图，按当前主题绘制；本地加载，保留 OSM 署名。
- 场景灵感入口与档案影响对照预览，保存卡片显示实际值与概念能力。
...
42项测试覆盖能力名、枚举、范围、步长、行驶限制、禁止值、无法表达的触发条件、负面偏好、局部修改、澄清、存储恢复、SSE分块、重试、错误与取消。

生成服务测试使用受控响应夹具，不代表真实模型的质量和时延。真实模型联调需自行配置密钥。12条 React DOM 交互旅程覆盖保存跳转、续改、追问、删除、行驶限制、档案持久化和连续提交竞争等路径；真实浏览器视觉验收和 WebMCP 环境验证尚未完成。
