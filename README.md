...
v18 版本继续以用户上传的 Figma Make「Card Stack」为视觉基础，保留炭黑、青柠、倾斜显示器和弹出动效；收紧方案卡，分离语音反馈，补齐手动偏好与场景管理。接入 114 条原子能力校验与 OpenRouter 场景生成接口。

1920×1080 显示器可切换驾驶员视角与正视阅读。主要正文 22–24px、标题 30–36px；完整提案 580px、命令栏 680px、侧栏 320px、应用编辑窗 680px。动作仍是同一张卡片里的行，空分组隐藏。

[旧版 v16 备份](https://scene-studio-v16-7e13263.vercel.app) · [备份标签](https://github.com/hadan8977/scene-studio-demo/tree/backup/v16-7e13263) · [上一版 v17 源码备份](https://github.com/hadan8977/scene-studio-demo/tree/backup/v17-9763e1b) · [本轮执行计划](docs/v18-product-plan.md) · [实现与验证记录](docs/v18-validation.md)

明确要求创建场景时，生成条件与组合动作。只描述处境时，先在卡外回应，卡片显示“查看方案”，确认用“好”；明确调灯调温则交给车控，不生成场景卡。方案支持手动编辑和局部续改，保存与应用分别操作。
...
重启开发服务器，在屏内“设置”面板刷新连接，切换到“真实 AI”。`OPENROUTER_MODEL` 可留空，默认选择候选清单中当前可用的首个主候选。模型候选来自候选清单与 OpenRouter 实时目录的交集。
...
- “查看方案 / 好 / 编辑 / 保存”，缺信息时追问；关闭未保存的方案即丢弃。
...
- 「场景建议」收纳未保存建议，支持查看、编辑、保存和删除；提供可用模板与手动新建，相似场景可更新、另存或取消。
- 档案影响对照预览，保存卡片显示实际值与概念能力。
- 停车完整提案与行驶三行摘要；车窗、灯光和禁止动作独立裁决。
- “它学会了什么”：两份初始档案，支持手动添加、直接修改数值、删除与恢复；按档案独立持久保存，影响后续生成，不改写已有存档。
- 浏览器本地保存、刷新恢复、重新打开修改。
- 模型选择、真实时延、裁决记录和结构化输出。

默认在真实徐汇滨江地图与原稿命令栏等待输入，通过全局模式标注示例或真实 AI。示例模式仅支持预设案例及少量局部续改，不会将预设数据冒充为模型输出。
...
| `tests/`                                               | 能力裁决、存储、流式生成等 55 项服务与规则测试 |
...
context 可额外传入 `preferences` 数组（最多24条，包含 id、primary、value、negative、可选 deleted）；仅接受受支持的偏好字段与合法取值，显式空数组表示不使用档案默认偏好。
...
55 项服务与规则测试覆盖能力名、枚举、范围、步长、行驶限制、禁止值、无法表达的触发条件、负面偏好、局部修改、澄清、存储恢复、SSE分块、重试、错误与取消。

生成服务测试使用受控响应夹具，不代表真实模型的质量和时延。真实模型联调需自行配置密钥。25 条 React DOM 交互旅程覆盖保存、续改、车控、主动询问、三秒预览、后续指令优先、拒绝冷却、收件箱、查重、档案手动增删改、刷新、手动创建、卡外语音和连续提交等。DOM 测试不是视觉验收；本轮浏览器环境不可用，实屏阅读与 WebMCP 环境验证尚未完成。
